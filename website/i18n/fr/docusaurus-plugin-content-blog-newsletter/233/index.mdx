...
Cette semaine, nous avons non pas un, mais deux articles de blog intéressants de Dan Abramov, ainsi que diverses versions de React.
...
Mes collègues chez ui.dev lancent également le contenu final de leur cours [react.gg](https://react.gg/?ref=twir) (30% de réduction actuellement), conçu pour vous enseigner tous les nouveaux concepts de React 19 à partir des principes fondamentaux. J'ai eu accès pour le relire et peux vous assurer que c'est une manière ludique d'apprendre React, remplie d'exemples interactifs, de visualisations et de quiz pragmatiques.
...
[![Réduisez le temps de revue de code React et les bugs de moitié](/emails/issues/233/coderabbit.jpg)](https://coderabbit.link/this-week-in-react-7may-2025)

**[Réduisez le temps de revue de code React et les bugs de moitié](https://coderabbit.link/this-week-in-react-7may-2025)**

Les revues de code sont cruciales mais chronophages. CodeRabbit agit comme votre co-pilote IA, fournissant des commentaires de revue de code instantanés et l'impact potentiel de chaque pull request.

Au-delà du simple signalement des problèmes, CodeRabbit propose des corrections en un clic et vous permet de définir des règles personnalisées de qualité de code en utilisant des motifs AST Grep, détectant des problèmes subtils que les outils d'analyse statique traditionnels pourraient manquer.

CodeRabbit a jusqu'à présent examiné plus de 5 millions de PR, est installé sur 1 million de dépôts et utilisé par 50 000 projets open source. CodeRabbit est gratuit pour tous les dépôts open source. [Commencez dès aujourd'hui !](https://coderabbit.link/this-week-in-react-7may-2025)
...
[Tolgee](https://tolg.ee/y1z3eb) est un outil **open-source** qui simplifie la **localisation** en faisant gagner du temps aux développeurs et en éliminant les interminables feuilles de calcul de texte à traduire 🌎

Accélérez votre travail dès maintenant ! Tolgee vous aide à :

- Déléguer la traduction à d'autres membres de l'équipe - économisez le temps du développeur !
- Traduction en contexte
- Cliquez pour traduire avec l'extension navigateur
- Se débarrasser des feuilles de calcul et JSON désorganisés

**Aucune carte de crédit n'est nécessaire** pour la version gratuite, parfaite pour les petits projets ! Vous pouvez essayer toutes les fonctionnalités et l'intégrer à votre projet immédiatement pour le préparer à rencontrer un public mondial.
...
Il s'agit d'une version assez impressionnante d'Expo, avec comme points forts :

- Mises à niveau vers React Native 0.79, React 19 et RNW 0.20
- Active la Nouvelle Architecture par défaut
- Le mode Edge-to-edge sur Android est activé par défaut
...
- Nouveau [Expo Maps](https://expo.dev/blog/introducing-expo-maps-a-modern-maps-api-for-expo-developers) (alpha) construit sur l'API Expo Modules, utilisant SwiftUI et Jetpack Compose, exposant les composants par défaut des plateformes Apple Maps et Google Maps.
- Résolution Metro `package.json#exports` activée par défaut
- Déployez les builds de développement sur TestFlight avec `npx testflight`
- Réduisez les temps de build Android jusqu'à 25% avec les modules Expo précompilés
- Et bien plus encore !

Voir aussi la sortie associée [Expo Router v5](https://expo.dev/blog/expo-router-v5) avec ses propres points forts :

- Nécessite React 19 / Expo 53
- Améliore l'expérience développeur (DX), l'écran d'erreur et la journalisation, tire parti des piles propriétaires de React
- Déployez des routes API versionnées en production avec EAS Hosting
- React Server Functions en bêta
- Routes protégées
...
Dans une vidéo de 37 minutes remplie d'annonces passionnantes, William Candillon et ses invités nous ont offert un aperçu du futur du dessin et de l'animation. Soutenu par Shopify, il apporte le support moderne de WebGPU (Vulkan et Metal) à React Native.

En s'alignant sur les APIs web, il est possible de copier/coller des exemples web existants (y compris Three.js) et de les faire fonctionner sur toutes les plateformes. Cela permet également une utilisation générale du GPU (IA, TensorFlow.js), réalisable plus facilement grâce à une bibliothèque bas niveau pratique : TypeGPU.

Tout ce travail était nécessaire pour créer un nouveau backend expérimental Skia Graphite basé sur WebGPU. Il permet de composer et d'entrelacer des expériences 2D/3D de manière performante, comme sur le web, mais sur toutes les plateformes, avec une intégration complète de Reanimated. Au passage, Skia n'apporte pas de nouvelle fonctionnalité, mais a considérablement amélioré les performances, réduit les plantages, et prend en charge 3 nouvelles plateformes : tvOS, macOS et Node. Continue de masser cette pieuvre William !
...
- 📖 [Documentation Node.js - Publication d'un package](https://nodejs.org/en/learn/modules/publishing-a-package) : Un nouveau guide officiel couvrant également les subtilités ESM/CJS.
...
- 📦 [Node 24.0](https://nodejs.org/en/blog/release/v24.0.0) : La nouvelle version de Node.js est disponible et deviendra LTS en octobre, incluant npm 11, des améliorations de performances pour `AsyncLocalStorage`, l'API `URLPattern` et une mise à jour majeure de V8 en v13.6 débloquant de nouvelles APIs JS comme la gestion explicite des ressources (`using`) ou `RegExp.escape()` (désormais [disponible partout](https://web-platform-dx.github.io/web-features-explorer/features/regexp-escape/)).
- 📦 [GSAP 3.13 - Bibliothèque d'animations JS - Désormais 100% gratuite, y compris tous les plugins](https://gsap.com/blog/3-13/)
- 📦 [Deno 2.3 - Améliorations de deno compile, paquets npm locaux, deno fmt dans les templates étiquetés, et plus](https://deno.com/blog/v2.3)
