...
Cette semaine, nous avons beaucoup d'actualités et de sorties intéressantes !

On note notamment un changement terminologique important pour les Server Actions et un aperçu des prochains React DevTools v6.

Côté React Native, nous avons également un large éventail d'actualités et d'aperçus de bibliothèques passionnantes !

De plus, mon jeu de mèmes est de retour. Vous comprendrez à la fin 😂

Découvrez notre conférence partenaire [React Day 🇩🇪 Berlin](https://reactday.berlin/?utm_source=Newsletter&utm_medium=thisweekinreact) (13 & 16 décembre). Bénéficiez d'une réduction de 10% avec notre code TWIR.
...
[![Suivez les composants {React} & réduisez la dette technique avec Omlet.](/emails/issues/203/omletad3x.png)](https://omlet.dev/?utm_campaign=partner_this-week-in-react_newsletter-first-oct-2024)

**[Suivez les composants \{React} & réduisez la dette technique avec Omlet.](https://omlet.dev/?utm_campaign=partner_this-week-in-react_newsletter-first-oct-2024)**

Découvrez comment les composants React sont utilisés dans vos équipes de développement. Avec Omlet, obtenez une visibilité complète sur votre bibliothèque de composants pour démontrer la valeur métier de votre design system, réduire la dette technique et suivre l'adoption dans les projets. Omlet vous aide à identifier où se cachent les composants obsolètes, repérer les props inutilisés, améliorer votre bibliothèque de composants et simplifier les migrations.

[Générez des rapports exploitables en moins de 5 minutes.](https://omlet.dev/?utm_campaign=partner_this-week-in-react_newsletter-first-oct-2024) Analysez l'utilisation du code et observez l'impact de votre design system dans le temps.
...
[![Les Server Actions ont été renommées Server Functions](/emails/issues/203/serverfunctions.jpg)](https://19.react.dev/reference/rsc/server-functions)

**[Les Server Actions ont été renommées Server Functions](https://19.react.dev/reference/rsc/server-functions)**

Ce n'est pas un changement de code, mais un changement terminologique important désormais officiellement publié sur la documentation React v19. Les Server Actions sont désormais considérées comme un sous-ensemble de la fonctionnalité plus large des Server Functions. Elles ne deviennent des actions que lorsqu'elles sont utilisées dans :

- `<form action>`
- `useActionState()`
- `startTransition()`

Ce changement clarifie que "use server" concerne l'exposition d'un endpoint serveur, pas nécessairement limité aux mutations. Récupérer des données et du JSX avec des Server Functions pourrait finalement ne pas être un anti-pattern.

**À propos des Server Actions de Next.js** : elles utilisent des requêtes `POST` et s'exécutent séquentiellement, mais c'est un détail d'implémentation de Next.js. React ne vous empêche pas d'exécuter des actions en parallèle. L'équipe Next.js a confirmé son intention d'introduire de nouvelles API pour appeler les Server Functions avec `GET`.
...
🔐 WorkOS AuthKit prend en charge une solution complète de gestion des utilisateurs avec **SSO, SCIM, RBAC & FGA**.
...
- 📨 [ECMAScript News - Ma newsletter préférée pour suivre ES, TC39, les outils JS et les runtimes](https://ecmascript.news/?utm_source=thisweekinreact)
- 📣 [Annonce de VoidZero \- Chaîne d'outils nouvelle génération pour JavaScript](https://voidzero.dev/posts/announcing-voidzero-inc) : Evan You a annoncé `void(0)`, une entreprise soutenue par du capital-risque pour développer une chaîne d'outils JS ultra-rapide incluant Vite, Vitest, Rolldown, Oxc. Ces projets resteront gratuits sous licence MIT.
- 👀 [PR Node.js \- débloquer require(esm) pour v23](https://github.com/nodejs/node/pull/55085) : Bonne nouvelle pour l'interopérabilité CJS/ESM de Node.js. Ce changement est prévu pour être rétroporté dans les anciennes versions LTS.
- 📜 [Les Web Components ne sont pas l'avenir](https://dev.to/ryansolid/web-components-are-not-the-future-48bh) : Une controverse sur les Web Components fait actuellement rage, avec de nombreux experts exprimant leurs opinions.
...
- 📦 [Oxc Transformer Alpha \- Transpilation 5x plus rapide que SWC](https://oxc.rs/blog/2024-09-29-transformer-alpha.html)
...
[![alternative](/emails/issues/203/meme.jpg)](https://x.com/sebastienlorber/status/1839661750498578602)

[![alternative](/emails/issues/203/meme2.jpg)](https://x.com/sebastienlorber/status/1840765064246112276)

[![alternative](/emails/issues/203/meme3.jpg)](https://x.com/sebastienlorber/status/1841110004792696850)
